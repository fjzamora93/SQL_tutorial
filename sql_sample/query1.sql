...
/*__NO FORMA PARTE DE LA PRUEBA: CÓMO SIMULAR UN MINUS EN MYSQL, solo son apuntes*/

/*Esta consulta nos debería devolver los departamentos sin empleados*/

UPDATE empleados
SET idpto = 1
	WHERE idpto IN (
		SELECT idpto FROM departamentos 
...
SELECT idpto, nombre
FROM departamentos
WHERE idpto NOT IN (SELECT idpto FROM empleados);
